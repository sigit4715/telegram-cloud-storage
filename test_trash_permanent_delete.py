...
assert 'SELECT id, msg_id FROM files WHERE id=? AND owner_email=? AND deleted_at IS NOT NULL' in src
assert 'get_telegram_client(owner_perm).delete_messages(telegram_target(owner_perm), [row[0]["msg_id"]])' in src
